...
contract TestJBBuybackDelegate_Units is Test {
  ...
  function setUp() external {
    ...
    vm.mockCall(
      address(jbxTerminal),
      abi.encodeCall(jbxTerminal.store, ()),
      abi.encode(terminalStore)
    );
  ...
  function test_didPay_swap(uint256 _tokenCount, uint256 _twapQuote, uint256 _reservedRate) public {
    ...
    vm.mockCall(
      ...
      abi.encodeCall(projectToken.transfer, (dude, _nonReservedToken)),
      abi.encode(true)
    );
    ...
    vm.mockCall(
      address(jbxTerminal),
      abi.encodeCall(jbxTerminal.directory, ()),
      abi.encode(address(directory))
    );
    vm.mockCall(
      address(directory),
    ...
    vm.mockCall(
      ...
      abi.encodeCall(
        ...
        (didPayData.projectId, IJBPaymentTerminal(address(jbxTerminal)))
      ),
      abi.encode(true)
    );
    ...
    vm.mockCall(
      ...
      abi.encodeCall(
        ...
        (address(delegate), didPayData.projectId, _twapQuote, '', true)
      ),
      abi.encode(true)
    );
  ...
  function test_didPay_keepTrackOfETHToSweep() public {
    ...
    vm.mockCall(
      address(projectToken),
      abi.encodeCall(projectToken.transfer, (dude, _twapQuote)),
      abi.encode(true)
    );
  ...
  function test_didPay_revertIfWrongCaller(address _notTerminal) public {
    ...
    vm.mockCall(
      ...
      abi.encodeCall(
        directory.isTerminalOf,
        (didPayData.projectId, IJBPaymentTerminal(address(_notTerminal)))
      ),
      abi.encode(false)
    );
  ...
  function test_uniswapCallback() public {
    ...
    (_delta0, _delta1) = address(projectToken) < address(weth)
      ? (_delta0, _delta1)
      : (_delta1, _delta0);
    ...
    vm.mockCall(
      ...
      abi.encodeCall(
        weth.transfer,
        (address(pool), uint256(address(projectToken) < address(weth) ? _delta1 : _delta0))
      ),
      abi.encode(true)
    );
  ...
  function test_uniswapCallback_revertIfMaxSlippage() public {
    ...
    vm.prank(address(pool));
    vm.expectRevert(
      abi.encodeWithSelector(JBBuybackDelegate.JuiceBuyback_MaximumSlippage.selector)
    );
    delegate.uniswapV3SwapCallback(_delta0, _delta1, abi.encode(_minReceived));
  }
  ...
  function test_increaseSecondsAgo_revertIfNoIncrease(uint256 _oldValue, uint256 _newValue) public {
    ...
    // check: revert?
    vm.expectRevert(
      abi.encodeWithSelector(JBBuybackDelegate.JuiceBuyback_NewSecondsAgoTooLow.selector)
    );
